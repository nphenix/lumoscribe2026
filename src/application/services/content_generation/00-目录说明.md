...
| `section_context.py` | 检索上下文组装工具（来源标题/去重/截断等） | 2026-01-28 |
| `recall_utils.py` | 并发召回与限流工具 | 2026-01-28 |
| `coverage_utils.py` | coverage/audit 条目构建工具 | 2026-01-28 |
| `postprocessors.py` | 章节后处理工具（语义去重/空条目清理） | 2026-01-29 |
| `pipeline.py` | 章节生成多阶段管线（生成→后处理→可选润色） | 2026-01-29 |
...
| 2026-01-28 | 新增 | `section_context.py` | 抽离上下文组装通用工具：标题判定/去重/截断/来源块 |
| 2026-01-28 | 新增 | `recall_utils.py` | 抽离并发召回与限流工具，复用进度回调能力 |
| 2026-01-28 | 新增 | `coverage_utils.py` | 抽离 coverage/audit 条目构造，减少 service 内重复字典拼装 |
| 2026-01-29 | 新增 | `pipeline.py` | 抽象章节生成多阶段管线，支持后处理与章节润色 |
| 2026-01-29 | 新增 | `postprocessors.py` | 新增语义去重与空条目清理的后处理工具 |
