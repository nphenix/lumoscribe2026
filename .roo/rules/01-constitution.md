...
- **自然语言优先**: 所有与 AI 助手的交互应使用自然语言描述意图（如"请审查代码"、"生成提交信息"），而非使用斜杠命令（如 `/review`）。
- **单一事实来源**: `speckit/` 目录是所有规范、模板和技能的唯一事实来源。
- **自动同步**: 对 `speckit/` 的任何修改都必须通过 `scripts/sync-skills.ps1` 脚本同步到工具配置目录（`.cursor/` 和 `.roo/`）。
...
**版本**: 1.4.0 | **批准日期**: 2026-1-11 | **最后修正**: 2026-1-12
